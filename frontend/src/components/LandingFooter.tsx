...
export default function LandingFooter() {
  return (
    <footer className="border-t border-slate-200 bg-white px-4 py-12 text-slate-700 dark:border-slate-400/15 dark:bg-[#0B1220] dark:text-slate-300 sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-7xl gap-8 text-center lg:grid-cols-[1.1fr_1.4fr_0.8fr] lg:items-start lg:text-left">
        <Link href="/" className="mx-auto flex items-center gap-3 lg:mx-0" aria-label="Dhune home">
          <span className="flex h-12 w-12 items-center justify-center rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-400/15">
        ...
        <div className="flex flex-wrap justify-center gap-x-5 gap-y-3 text-sm font-bold lg:justify-center">
          {quickLinks.map((link) => (
            <a key={link.href} href={link.href} className="transition hover:text-[#040947] dark:hover:text-white">
              {link.label}
            </a>
          ))}
        </div>
        ...
        <div className="flex flex-col items-center gap-3 text-sm lg:items-end">
          <a href="mailto:support@dhune.np" className="inline-flex items-center gap-2 font-bold transition hover:text-[#040947] dark:hover:text-white">
