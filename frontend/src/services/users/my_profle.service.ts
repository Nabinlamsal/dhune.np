...
import { UserDocument } from "@/src/types/users/document";
...
const normalizeMyProfile = (raw: unknown): AdminUserProfile => {
    ...
    return {
        ...
        Documents: documentsRaw.map((document) => {
            const item = (document ?? {}) as Record<string, unknown>;

            return {
                ID: pick<string>(item, ["ID", "id"]) ?? "",
                DocumentType: pick<string>(item, ["DocumentType", "document_type", "documentType"]) ?? "",
                DocumentURL: pick<string>(item, ["DocumentURL", "document_url", "documentUrl"]) ?? "",
                Verified: Boolean(pick<boolean>(item, ["Verified", "verified"])),
                CreatedAt: pick<string>(item, ["CreatedAt", "created_at", "createdAt"]) ?? "",
...
export const uploadProfileImage = async (file: File): Promise<AdminUserProfile> => {
